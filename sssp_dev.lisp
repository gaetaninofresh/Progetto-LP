...
(defun shortest-path (graph-id source-id vertex-id)
  (let* (
            ...
            (distance (get-edge-cost graph-id prev-id vertex-id))
        ) 
        (if (null prev-id)
            (progn
                (format t "~% No path from ~S to ~S ~%" source-id vertex-id)
            )
            ...
            (if (equal source-id vertex-id)
                ...
                ;else
                (progn
                    (push (list 'edge graph-id prev-id vertex-id distance) 
                        (list (shortest-path graph-id source-id vertex-id))
                    )
                )
            )
        )
...
(defun dijkstra-heap-insert (heap-id vertex-id distance)
    (let (
        ...
        (if (null key)
            ...
            ;else
            (modify-key heap-id distance key vertex-id)
        )
    )
)
